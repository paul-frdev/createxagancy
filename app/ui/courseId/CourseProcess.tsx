          ...
          {items.map(item => (
            <div key={item.id} className="flex lg:block w-full max-w-[285px] h-[161px] lg:h-[218px]">
              <div className="w-full flex justify-start mb-4 relative h-[90px]">
                {item.id !== 4 ? <span className='absolute top-[124px] lg:top-1/2 rotate-90 lg:rotate-0 right-[16px] lg:right-0 border border-dashed border-gray400 w-full max-w-[210px]' /> : null}
                <span className={cn(`flex justify-center items-start lg:items-center text-[2.875rem] text-gray400 text-left font-lato font-bold`, item.id === 4 ? 'w-[68px] lg:w-[84px] h-[68px] lg:h-[84px] text-orange bg-primary bg-opacity-[0.12] rounded-full animation-pulse -ml-3 lg:-ml-0 mt-3 lg:mt-0' : '')}>{item.number}</span>
              </div>
