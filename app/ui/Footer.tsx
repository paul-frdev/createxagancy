    ...
    <footer className=" bg-gray900 w-full pt-14">
      <Container className="flex flex-col xsm:flex-row flex-wrap w-full justify-between items-center xsm:items-start mb-12">
        <div className="flex flex-col gap-y-6 justify-start items-center xsm:items-start w-full max-w-[286px]">
          ...
          <Typography className="text-white text-center xsm:text-left text-[0.75rem] normal-case opacity-[0.6] font-lato font-normal">
          ...
          <div className="flex justify-between items-center gap-x-6">
            {socials.map((item) => (
              <Link key={item.id} href="/" className="w-6 h-6 text-bgIcon hover:text-white transition-all duration-300">
                {item.icon}
              </Link>
            ))}
          </div>
          ...
          <ul className="flex flex-col justify-start items-center xsm:items-start gap-y-[0.7rem]">
            {footerLinks.map((item) => (
              <li key={item.id}>
                <Link
                  href="/"
                  className="text-base text-nowrap text-white opacity-[0.6] hover:opacity-[1] transition-all duration-300 leading-[160%] tracking-[1px] font-lato font-normal"
                >
                  {item.title}
                </Link>
              </li>
        ...
        <div className="flex flex-col gap-y-6 justify-start items-center xsm:items-start w-full max-w-[286px] [@media(max-width:1073px)]:mt-6">
          ...
          <EmailForm />
          <span className="text-[0.625rem] text-center xsm:text-left text-white opacity-[0.6] font-lato font-normal leading-[150%]">
      ...
      <div className="flex flex-col sm:flex-row justify-center sm:justify-between items-center w-full h-auto py-3 sm:py-0 sm:h-[58px] bg-bgFooter">
        <div className="flex  justify-center sm:justify-start items-center">
          ...
          <Typography className=" text-nowrap text-[0.75rem] font-lato font-normal normal-case text-white mb-0 ml-1">by Createx Studio</Typography>
        </div>
        <Button onClick={scrollToTop} ref={buttonRef} className="text-nowrap p-0 py-2 px-6 bg-transparent border-none hover:bg-transparent hover:border-none opacity-[0.6]">
          GO TO TOP
        </Button>
      </div>
    </footer>
